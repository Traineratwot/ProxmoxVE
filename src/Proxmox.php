...
class Proxmox {
	 ...
	 * Constructor.
	 *
	 * @param mixed              $credentials Credentials object or associative array holding the login data.
	 ...
	 * @throws \ProxmoxVE\Exception\MalformedCredentialsException If bad args supplied.
	 * @throws \ProxmoxVE\Exception\AuthenticationException If given credentials are not valid.
	 */
	public function __construct(
		...
		// Set the response type. Default: array.
	 ...
	 *
	 * @param string $actionPath The resource tree path you want to request.
	 * @param array  $params An associative array filled with params.
	 * @param string $method HTTP method used in the request. Default is 'GET'.
	 * @return \Psr\Http\Message\ResponseInterface
	 * @throws \InvalidArgumentException If the given HTTP method is not one of 'GET', 'POST', 'PUT', 'DELETE'.
	 */
	 ...
	 * @return mixed The parsed response, depending on the response type can be an array or a string.
	 * @throws BadResponseException Bad Response Exception.
	 */
	...
	/**
	 * Sets the HTTP client to be used to send requests over the network, for now Guzzle needs to be used.
	 *
	...
	/**
	 * Attempts to login using set credentials, if succeeded will return the AuthToken used in all requests.
	 *
	 * @return \ProxmoxVE\AuthToken When successful login will return an instance of the AuthToken class.
	 ...
	 *
	 * @return \ProxmoxVE\Credentials Object containing all proxmox data used to connect to the server.
	 */
	 ...
	 *
	 * @param object $credentials A custom object holding credentials or a Credentials object to assign.
	 ...
	 *
	 * @param string $actionPath The resource tree path you want to ask for, see more at http://pve.proxmox.com/pve2-api-doc/.
	 * @param array  $params      An associative array filled with params.
	...
	public function createStorage( $storageData) {
		if (!is_array( $storageData )) {
			throw new \InvalidArgumentException( 'Storage data needs to be array' );
		}
